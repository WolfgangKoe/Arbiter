...
| [B-102](backlog_details.md#b-102--counter-offensive-box-selten-erreichbar)     | ToDo | <a id="b-102"></a>Counter-Offensive-Box selten erreichbar — `_advance_fight_turn_if_needed` kippt `fight_current_player` in einfachen 1-gegen-1-Sequenzen sofort auf die berechtigte Seite, wodurch `is_my_turn` True wird und die reaktive Box gar nicht rendert (Hypothesis A bestätigt S155); sichtbar nur wenn die Alternierung nicht sofort zurückspringt (z. B. Group-Assignment-Flow). E1-Zusatzbefund S155, `_enemy_has_fought` selbst ist korrekt.<br><span style="color:#166534">**Fachlichkeit (Ziel 7)**</span> | Zu klären: Soll die Box in diesen Sequenzen anders angeboten werden (z. B. eigener Moment vor dem Rücksprung)? | ~M | Executor |

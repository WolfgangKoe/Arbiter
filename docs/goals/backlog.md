...
- 🔴 **BUG Heroic Intervention crasht (S78, Ork-YAML):** Button-Betätigung in der Heroic-Intervention/Engage-Auswahl ließ die **komplette Ork-Armeeliste verschwinden**, App danach handlungsunfähig (kein Weiterkommen). Klingt nach demselben Muster wie der frühere FightPhase-Crash (`int("User×N")`/Center-Column-Exception). **Tests fehlen** für den Heroic-Intervention-/Engage-Flow. → Root-Cause-Analyse + Regressionstest. Eigener Bugfix-Plan.
- 🔴 **BUG Badge zeigt Wert doppelt „AP-4 -4" (S78):** `_aligned_modifier_row_html` baut `f"{label} {sign}{abs(value)}"`; bei AP-Waffen kodiert `label` (`AP{ap}` → „AP-4") den Wert bereits, das angehängte `-4` ist redundant → „AP-4 -4". Fix: Wert nicht doppeln (Label trägt ihn schon bei AP; ggf. nur anhängen, wenn Label keinen Zahlenwert enthält). Test: Badge-Text enthält den Wert genau einmal.
- 🔴 **BUG Buff-Badge nicht grün (S78, Light Cover):** Der Pfeil ist nach S78 grün, aber die **Badge-Chip-Farbe** nutzt `right_color`, das bei Buffs (SAVE) grau ist → Light Cover wird grau statt grün gerendert. **Farb-Test fehlt** (kein Test pinnt die Badge-Farbe). Zugleich offene Konzeptfrage: **Perspektive** — aus wessen Sicht ist ein Modifier Buff/Debuff (Verteidiger-Cover = Buff)? `color_hint` existiert (S77), wird aber für die SAVE-Badge nicht ausgewertet. → Badge-Farbe an Modifier-/`color_hint`-Farbe binden + Perspektiv-Konzept einplanen + Tests.

...
- [x] **P17:** ✅ Pre-Apply-Zielauswahl (`_render_subgroup_selector`, `_common.py`) + Wounded-Lock (`apply_damage`/`get_locked_group`, `unit_mutations.py`): Spieler wählt vor „Apply" die Zielgruppe; ein angeschlagenes Modell bleibt gelockt, bis es zerstört ist, dann Neuauswahl. Getestet: `test_lock_invariante_*`, `test_group_flow.py`. Stakeholder-Entscheidung S115: Vor-Auswahl-Variante akzeptiert, ±-Zähler verworfen.
- [x] **P18:** ✅ (verifiziert S117 gegen `git log`; umgesetzt in `e6fcdb3` „Unify attack declaration on the group flow (Plan 013)", S43) — Legacy-Pfad `render_attack_declaration` entfernt; alle Einheiten routen über `render_group_cards`/`render_group_assignment` (`_common.py`), Einheiten ohne YAML-`model_groups` bekommen eine synthetische Einzelgruppe; `melee_with`-Ziele erscheinen bei selektierter Gruppe. Getestet: `test_group_flow.py` (34). Checkbox war stale-offen.
  → Ziel7-Cluster
...
- **S119 (2026-07-03)** Ziel7-Cluster P19/P20/P21 gefixt + UI-verifiziert; Ziel6 geschlossen; Ziel7-Restruktur Stufe A-C geplant und freigegeben

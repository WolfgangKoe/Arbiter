...
- **S47 (2026-06-13, Commit `e98d89d`)** Manuelle Verifikation → Findings G1–G5. G1 (Granaten-Cap pro Einheit), G5 (RP einmal pro Verteidiger nach voller Abhandlung der angreifenden Einheit, generisch), G4 (Reset löscht stale `decl_*`-Widget-Keys — re-verifizieren), G3 teilweise (`necrons_test.yaml`). +1 Test, 798 grün. **G2 (Silent King) blockiert:** braucht per-Gruppe-Wunden-Subsystem + fehlendes Triarchal-Menhir-Profil (Scraper-Lücke) — Details in `next_session.md`.

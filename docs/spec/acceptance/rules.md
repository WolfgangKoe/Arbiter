...
- **getestet**: ja — test_find_unit_ability_by_effect_real_data / test_explode_fields_real_data / test_check_conditions_passes_for_the_silent_king / test_vengeance_flow_explode_gate_applies / test_vengeance_flow_does_not_explode / test_rule_text_includes_it_explodes / test_exploded_true_returns_true_without_rolling / test_exploded_false_returns_false_without_rolling / test_raises_for_non_explode_ability / test_require_explode_effect_shape_rejects_missing_roll_threshold / test_require_explode_effect_shape_rejects_missing_radius / test_require_explode_effect_shape_rejects_missing_damage / test_require_explode_effect_shape_accepts_complete_ability / test_load_unit_abilities_rejects_incomplete_explode_ability / test_real_data_explode_carriers_have_correct_wahapedia_values / test_vengeance_of_the_enchained_flow_trigger_resolve_apply / test_mortal_wounds_on_destroy_card_hidden_for_silent_king_now_explode_typed / test_mortal_wounds_on_destroy_card_hidden_while_unit_alive / test_mortal_wounds_on_destroy_card_hidden_for_unit_without_the_ability / test_render_mortal_wounds_cards_for_destroyed_scans_both_factions / test_render_mortal_wounds_cards_for_destroyed_reaches_destroyed_unit_independent_of_selection / test_render_active_shows_card_despite_group_flow_early_return
- **quelle**: wahapedia_necrons/units_all.txt:238/689 — Vengeance of the Enchained (The Silent King): "When Szarekh is destroyed, roll one D6 before removing it from play. On a 4+ it explodes, and each unit within 2D6" suffers D6 mortal wounds." Wortlaut stimmt jetzt vollständig (inkl. "it explodes") mit `data/wh40k_9e/necrons/unit_abilities.yaml:vengeance_of_the_enchained` überein — die zuvor verkürzte rule_text (fehlendes "it explodes", B-028c1-Backlog-Befund) wurde in b1 korrigiert.
- **code**: `gameObjects/ability.py:Effect` (Felder `radius`/`damage`, Ability-Feld `mandatory`) · `gameObjects/loader.py:_ability_from_dict` + `_require_explode_effect_shape` (Schema-Validierung, lädt loud-fail bei fehlendem roll_threshold/radius/damage) · `gameMechanic/abilityEngine.py:resolve_explode_effect` (App würfelt nicht — nimmt den am Tisch gewürfelten Gate-Ausgang entgegen, kein `random`/`parse_dice` im Explode-Pfad; Rückbau von `resolve_mortal_wounds_effect`'s Selbstwurf für diese Ability, B-028c1 b1) · `unitMutations.py:apply_mortal_wounds` (unverändert, b2-Zielpfad). Alte UI (`uiLayout/_common.py:_render_mortal_wounds_on_destroy_card` / `render_mortal_wounds_cards_for_destroyed`) sucht weiterhin `effect.type: mortal_wounds` und findet Silent King daher nicht mehr — bewusst: die Use/Undo-GO-Bauform ist als falsch erkannt (S165) und wird durch die Pflicht-Trigger-Kachel in B-028c1 b2 ersetzt, kein neuer Call-Site in b1.
- **regel**: `effect.type: explode` (Explodes-Familie, Pflicht-Trigger, B-028c1 b1, S169) ersetzt das bisherige `effect.type: mortal_wounds` für Vengeance of the Enchained — neue Schema-Achse `Ability.mandatory: true` markiert Pflicht-Trigger (kein Use/Undo-GO). `Effect` trägt `roll_threshold`/`radius`/`damage` statt `target`/`amount`: Spatial-Auflösung ("units within 2D6\"") UND die Mortal-Wound-Anzahl sind vollständig Tisch-/UI-Sache (Klasse-A-App-Anteil bleibt nur die Datenhaltung + die Gate-Anwendung über `resolve_explode_effect`, die selbst nicht würfelt). **Datenbug-Historie (S164 T2, weiterhin gültig):** `conditions: []` — Ownership via `unit_id` ist der volle Gate (mirrors `noctilith_beacons`). **Re-Scope S165/b1:** fachlich als Pflicht-Trigger (Explodes-Familie) klassifiziert; die alte GO-Karten-Bauform (Use/Undo) ist retired, UI-Ersatz (Pflicht-Trigger-Kachel) folgt in b2 — bis dahin zeigt die App für Silent King keine Karte (dokumentierte Zwischenlücke, kein Regressionsrisiko da vorher ohnehin die falsche Bauform lief).
...
- **regel**: `effect.type: explode`, `roll_threshold: 6`, `radius: "6"`, `damage: "D3"`, `mandatory: true`. Einzelmodell-Einheit (models_min == models_max == 1) — "model destroyed" == "unit destroyed", passt auf den bestehenden `unit_state.destroyed`-Flag ohne Erweiterung (B-028c1 b1).
...
- **regel**: `effect.type: explode`, `roll_threshold: 6`, `radius: "3"`, `damage: "1"`, `mandatory: true`. Einzelmodell-Einheit, gleiche Begründung wie R-COMBAT-41.
...
- **regel**: `effect.type: explode`, `roll_threshold: 6`, `radius: "6"`, `damage: "D3"`, `mandatory: true`. Einzelmodell-Transport (models_min == models_max == 1), gleiche Begründung wie R-COMBAT-41.

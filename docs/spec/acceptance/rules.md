...
- **code**: uiLayout/_common.py:render_reactive_stratagem_box (canonical GO card, `effect_type="reroll"`) — aufgerufen von uiLayout/_common.py:_render_damage_block (Damage-Wurf, nach Apply gesperrt) und gameMechanic/psychicPhase.py:_render_psi_result / _render_undo_deny_button (Psychic Test/Manifest, Deny the Witch, S135 Paket 4b). uiLayout/_common.py:render_inline_command_reroll (Pull-statt-Push, kein Lock-Zustand) bleibt für Felder ohne "applied"-Sperre: gameMechanic/chargephase.py:114 (Charge-Wurf) und uiLayout/_common.py:render_group_assignment (Anzahl-Attacken, `decl_a_*`-Feld, S135 Paket 4c)
- **regel**: Command Re-Roll (1 CP, Core-Stratagem, `player: both`) — sichtbar nur solange CP≥1, in dieser Phase noch nicht verwendet, und der Wurf noch "der letzte" ist. Von den 9 regelerlaubten Wurf-Arten sind **4 verdrahtet**: Damage-Wurf, Psychic Test/Manifest und Deny the Witch (alle drei seit S135 Paket 4b als GO-Karte — `render_reactive_stratagem_box`, Reopen via `on_resolved`), sowie Anzahl-Attacken (S135 Paket 4c — `render_group_assignment`, melee "‹Weapon› — Attacks"-Feld, `on_reroll` ist ein No-op, da das Feld vor "Group done" nie gesperrt ist). **5 offen**: Hit-, Wound- und Save-Wurf sind in `_render_resolution_tab` reine Referenzanzeigen (Schwellenwert + Würfelbild) ohne eigenen erfassten Wert — die S135-4a/4b-Anker an diesen drei Stellen reagieren auf ein anderes Ereignis (`on_target`, vor dem Wurf, `effect_type` debuff_roll/invuln_save) und schließen `effect_type="reroll"` bewusst aus, decken Command Re-Roll also nicht ab; nur der kollabierte Schadenswert wird gespeichert. Advance-Wurf (movementPhase.py, migriert auf `render_go_card`) und Charge-Wurf (chargephase.py) speichern weiterhin keinen numerischen Wurfwert (nur `movement_choice`/`in_melee`-Flags) — es gibt nichts Gesperrtes zum Wiedereröffnen, das bespoke Inline-Angebot bleibt die bewusste §6.3-Ausnahme. Diese 5 bleiben eine dokumentierte Schuld (kein automatischer Ledger-Eintrag, da `status: implementiert` mit Test korrekt bleibt für die verdrahteten Wurf-Arten).

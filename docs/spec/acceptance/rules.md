...
- **klasse**: A
- **status**: implementiert
- **getestet**: ja — test_command_re_roll_has_phase_reactive_timing / test_command_reroll_visible_and_clickable_spends_cp / test_damage_block_offers_command_reroll_after_apply / test_manifest_reroll_offered_when_failed / test_deny_reroll_offered_when_deny_roll_was_made
...
- **code**: uiLayout/_common.py:render_inline_command_reroll (CP/usage-Gate + Spend, data-driven über `event: after_roll`), aufgerufen von uiLayout/_common.py:_render_damage_block (Damage-Wurf) und gameMechanic/psychicPhase.py:_render_psi_result / _render_undo_deny_button (Psychic Test, Deny the Witch)
- **regel**: Command Re-Roll (1 CP, Core-Stratagem, `player: both`) — S130 verdrahtete es als nicht-blockierendes Inline-Angebot (Pull statt Push, kein Use/Pass-Dialog) neben dem Wurfergebnis, sichtbar nur solange CP≥1, in dieser Phase noch nicht verwendet, und der Wurf noch "der letzte" ist. Von den 9 regelerlaubten Wurf-Arten sind **3 verdrahtet**: Damage-Wurf (`_render_damage_block`, Attacker zahlt — der einzige Wurf, der in dieser App als eigener, nach Apply gesperrter Wert existiert), Psychic Test/Manifest und Deny the Witch (`psychicPhase.py`, mit korrekter Zeitfenster-Gate: Manifest nur solange kein späterer Deny-Wurf oder bereits angewandter Perils-Schaden das Fenster geschlossen hat). **6 offen** (kein natürlicher, im Session-Budget erreichbarer Sperr-/Wiederöffnen-Punkt): Hit-, Wound- und Save-Wurf sind in `_render_resolution_tab` reine Referenzanzeigen (Schwellenwert + Würfelbild) ohne eigenen erfassten Wert — nur der kollabierte Schadenswert wird gespeichert; Anzahl-Attacken (`decl_a_*`/`decl_m_*`) ist nur grob über "Group done" sperrbar (kein Einzelfeld-Reopen ohne größeren Umbau); Advance-Wurf (movementPhase.py) und Charge-Wurf (chargephase.py) speichern gar keinen numerischen Wurfwert (nur `movement_choice`/`in_melee`-Flags) — es gibt nichts Gesperrtes zum Wiedereröffnen. Diese 6 bleiben eine dokumentierte Schuld (kein automatischer Ledger-Eintrag, da `status: implementiert` mit Test korrekt bleibt für die verdrahteten Wurf-Arten).

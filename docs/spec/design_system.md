...
| Reroll | ↺ | SVG-Würfelfläche | gleiche 32×32-Box (Rahmen 1.5px, `rx=4`), Inhalt statt Kreuz/Pip = zentriertes ↺-Glyph (`SYM_RESET`) | Buff-Grün `#4a9a5a` (`_BUFF_COLOR_HEX`, `design_colors.md` §0 — Reroll-von-1 ist ein Vorteil der ausführenden Einheit, kein neues Token) | `reroll_marker_row_html` → `_marker_row_html` (verdrahtet in HIT- und WOUND-Block, B-113 S178: Skorpekh-Destroyer Hit-Reroll-1 „Hardwired for Destruction" + Destroyer-Lord Wound-Reroll-1-Aura „United in Destruction"; Marker-Farbe Buff→Grün umgestellt in B-130 S179) |

...
| Reroll | ↺ | SVG-Würfelfläche | gleiche 32×32-Box (Rahmen 1.5px, `rx=4`), Inhalt statt Kreuz/Pip = zentriertes ↺-Glyph (`SYM_RESET`) | fix Reroll-Orange `#f59e0b` (bereits etablierte Reroll-Farbe, kein neues Token) | `reroll_marker_row_html` → `_marker_row_html` (verdrahtet in HIT- und WOUND-Block, B-113 S178: Skorpekh-Destroyer Hit-Reroll-1 „Hardwired for Destruction" + Destroyer-Lord Wound-Reroll-1-Aura „United in Destruction". Offener Folge-Befund: Buff→Grün statt Orange, S178-UI-Verifikation) |

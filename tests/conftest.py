...
_BRIEFING_BUDGET = 120  # Decke, keep in sync with tests/docs/test_doc_health.py
_BRIEFING_TRIM_TARGET = 70  # beim Reissen der Decke auf <= dieses Ziel kuerzen
...
def _metrics() -> list[str]:
    ...
    briefing = _ROOT / ".claude" / "tasks" / "briefing.md"
    n_lines = len(briefing.read_text(encoding="utf-8").splitlines())
    lines.append(f"INV-5  Akzeptanzkriterien          : {len(spec_ac_ids())} AC-IDs (alle gepinnt)")
    lines.append(
        f"INV-5  briefing.md                 : {n_lines}/{_BRIEFING_BUDGET} Zeilen "
        f"(Ziel beim Kuerzen: <= {_BRIEFING_TRIM_TARGET})"
    )

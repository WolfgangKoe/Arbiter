"""Tests for the Command Protocol slot swap (setup reordering).
...
from uiLayout.gameActionsArea import _protocol_slots_after_swap  # noqa: E402
...
def test_swap_between_two_rounds_exchanges_them() -> None:
    ...
    result = _protocol_slots_after_swap(slots, 2, "p4")
    assert result[2] == "p4"
    assert result[4] == "p2"
...
def test_swap_round_with_extra_slot() -> None:
    slots = {"extra": "p6", 1: "p1", 2: "p2", 3: "p3", 4: "p4", 5: "p5"}
    # Move the 6th protocol into round 1 → round 1's old protocol becomes the 6th.
    result = _protocol_slots_after_swap(slots, 1, "p6")
    assert result[1] == "p6"
...
def test_selecting_same_value_is_a_noop() -> None:
    ...
    assert _protocol_slots_after_swap(slots, 1, "p1") == slots


def test_original_map_is_not_mutated() -> None:
    slots = {"extra": "p6", 1: "p1", 2: "p2"}
    _protocol_slots_after_swap(slots, 1, "p2")

...
_HUNGRY_VOID = "wh40k_9e.necrons.faction.protocol_hungry_void"  # primary: hit +1 (shooting)
...
def test_mirror_match_protocol_modifiers_are_player_independent() -> None:
    ...
    session[round_choice_state_key("P1", "active")] = _HUNGRY_VOID
    ...
    assert get_active_round_choice_modifier("P1", "shooting", False) == {"hit": 1}
...
def test_reset_clears_each_player_independently() -> None:
    session = _mirror_session()
    for player, protocol in (("P1", _HUNGRY_VOID), ("P2", _ETERNAL_GUARDIAN)):
